...
"""
RiceRocks (Asteroids - DESTROYER)

My retouched solution of the mini-project #8 of the course
https://www.coursera.org/course/interactivepython (Coursera).
...
class RiceRocks:
    ...
    def draw_and_update(self, canvas):
        ...
        """ Draw background """
        # Level 1
        canvas.draw_image(self.medias.get_image('nebula1'),
                          self.img_infos['nebula1'].get_center(), self.img_infos['nebula1'].get_size(),
                          (SCREEN_WIDTH / 2.0, SCREEN_HEIGHT / 2.0), (SCREEN_WIDTH, SCREEN_HEIGHT))

        # Level 2
        if self.score >= 100:
            self.level = 2
            canvas.draw_image(self.medias.get_image('nebula2'),
                              self.img_infos['nebula2'].get_center(), self.img_infos['nebula2'].get_size(),
                              (SCREEN_WIDTH / 2.0, SCREEN_HEIGHT / 2.0), (SCREEN_WIDTH, SCREEN_HEIGHT))

        # Level 3
        if self.score >= 200:
            self.level = 3
            canvas.draw_image(self.medias.get_image('nebula3'),
                              self.img_infos['nebula3'].get_center(), self.img_infos['nebula3'].get_size(),
                              (SCREEN_WIDTH / 2.0, SCREEN_HEIGHT / 2.0), (SCREEN_WIDTH, SCREEN_HEIGHT))

        # Level 4
        if self.score >= 300:
            self.level = 4
            canvas.draw_image(self.medias.get_image('nebula4'),
                              self.img_infos['nebula4'].get_center(), self.img_infos['nebula4'].get_size(),
                              (SCREEN_WIDTH / 2.0, SCREEN_HEIGHT / 2.0), (SCREEN_WIDTH, SCREEN_HEIGHT))

        # Level 5
        if self.score >= 400:
            self.level = 5
            canvas.draw_image(self.medias.get_image('nebula5'),
                              self.img_infos['nebula5'].get_center(), self.img_infos['nebula5'].get_size(),
                              (SCREEN_WIDTH / 2.0, SCREEN_HEIGHT / 2.0), (SCREEN_WIDTH, SCREEN_HEIGHT))

        # Kada bude GAME OVER vracamo staru pozadinu i postavljamo level 1
        if self.lives <= 0:
            canvas.draw_image(self.medias.get_image('nebula1'),
                              self.img_infos['nebula1'].get_center(), self.img_infos['nebula1'].get_size(),
                              (SCREEN_WIDTH / 2.0, SCREEN_HEIGHT / 2.0), (SCREEN_WIDTH, SCREEN_HEIGHT))
        ...
        canvas.draw_text(s1, (SCREEN_WIDTH - 372 - width1, 22 + size * 3.0 / 4), size, 'Gray', font)
        canvas.draw_text(s2, (SCREEN_WIDTH - 347 - width2, 22 + size * 3.0 / 4), size, 'Gray', font)

        canvas.draw_text(s1, (SCREEN_WIDTH - 370 - width1, 20 + size * 3.0 / 4), size, 'White', font)
        canvas.draw_text(s2, (SCREEN_WIDTH - 345 - width2, 20 + size * 3.0 / 4), size, 'White', font)
        ...
        canvas.draw_text(s1, (SCREEN_WIDTH - 32 - width1, 22 + size * 3.0 / 4), size, 'Gray', font)
        canvas.draw_text(s2, (SCREEN_WIDTH - 32 - width2, 22 + size * 7.0 / 4), size, 'Gray', font)

        canvas.draw_text(s1, (SCREEN_WIDTH - 30 - width1, 20 + size * 3.0 / 4), size, 'White', font)
        canvas.draw_text(s2, (SCREEN_WIDTH - 30 - width2, 20 + size * 7.0 / 4), size, 'White', font)

        # High score
        size = 22
        s3 = 'High score'
        width1 = frame.get_canvas_textwidth(s1, size, font)
        s4 = str(self.high_score)
        width2 = frame.get_canvas_textwidth(s2, size, font)

        canvas.draw_text(s3, (SCREEN_WIDTH - 65 - width1, 22 + size * 18.0 / 4), size, 'Gray', font)
        canvas.draw_text(s4, (SCREEN_WIDTH - 35 - width2, 22 + size * 22.0 / 4), size, 'Gray', font)

        canvas.draw_text(s3, (SCREEN_WIDTH - 63 - width1, 20 + size * 18.0 / 4), size, 'White', font)
        canvas.draw_text(s4, (SCREEN_WIDTH - 33 - width2, 20 + size * 22.0 / 4), size, 'White', font)

        # Draw splash screen if game not started
        if not self.started:
            # Draw ship uder splash screen
    ...
    def load_medias(self):
        ...
        self.img_infos = {'asteroid-1': ImageInfo((45, 45), (90, 90), 40),
                          ...
                          'nebula1': ImageInfo((400, 300), (800, 600)),
                          'nebula2': ImageInfo((400, 300), (800, 600)),
                          'nebula3': ImageInfo((400, 300), (800, 600)),
                          'nebula4': ImageInfo((400, 300), (800, 600)),
                          'nebula5': ImageInfo((400, 300), (800, 600)),
        ...
        self.medias.add_image('http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/shot2.png',
                              'missile')
        self.medias.add_image('http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/nebula_blue.png',
                              'nebula1')  # pozadina za level:1
        self.medias.add_image('http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/nebula_brown.png',
                              'nebula2')  # pozadina za level:2
        self.medias.add_image('http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/double_ship.png',
                              'nebula3')  # pozadina za level:3
        self.medias.add_image('http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/nebula_brown.png',
                              'nebula4')  # pozadina za level:4
        self.medias.add_image('http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/nebula_blue.png',
                              'nebula5')  # pozadina za level:5
        self.medias.add_image('http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/double_ship.png',
                              'ship')
        self.medias.add_image(
            'http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/explosion_orange.png',
            'ship_explosion')
        self.medias.add_image('http://oi57.tinypic.com/2u3y1z7.jpg',
                              'splash')
